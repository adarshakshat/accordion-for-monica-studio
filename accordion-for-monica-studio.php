<?php
/**
 * Plugin Name: Accordion for Monica Studio
 * Plugin URI: http://imadarshakshat.tech/
 * Description: Changes the default WooCommerce tabs to an Accordion when on mobile devices
 * Version: 1.0.0
 * Author: Adarsh Akshat
 * Author URI: http://imadarshakshat.tech/
 *
 * Text Domain: wctta
 *
 * @author adarshakshat
...
class WooCommerceTabsToAccordion
{
	...
	{
		$sections['wctta'] = __('Product Tabs to Accordion', 'wctta');
		return $sections;
	}
	/**
	...
	public function register_plugin_settings( $settings, $current_section )
	{
		if ( $current_section == 'wctta' ) {
			...
			$settings_wctta[] = array(
				...
				'name' 	=> __('Product Tabs to Accordion', 'wctta'),
				'type' 	=> 'title',
	...
	public function register_scripts_and_styles()
	{
		...
		wp_register_style('accordion-for-monica-css', plugins_url('/css/accordion-for-monica.css', __FILE__));
		wp_register_script('accordion-for-monica-js', plugins_url('/js/responsive-tabs.js', __FILE__), array('jquery'));
		wp_enqueue_style('accordion-for-monica-css');
		wp_enqueue_script('accordion-for-monica-js');
	}
